...
/*     console.log("volumeLoader2 :", volumeLoader); */
    // 렌더링 엔진과 뷰포트 설정
  ...
  content.appendChild(viewportGrid);
    ...
    console.log("viewportInput :", viewportInput);
...
export default function Viewport() {
  ...
  const defaultConfig = {
    extensions: [],
      ...
      const setBothRefs = (element) => {
        volume1.current = element;
        volume2.current = element;
      };

    useEffect(() => {
    if (window !== undefined) {
      // 초기화 및 매니저 설정
      run(setBothRefs.current);
